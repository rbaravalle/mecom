...
Following the ideas presented in \cite{Gonzales2008}, the mentioned fractal and multifractal characteristics were obtained for each image (using $20$ H\"older exponents, a $42$ characteristics vector was obtained). The code of the algorithms Box dimension, Morphological fractal dimension and the multifractal spectrum was written and run in Matlab. In order to make a comparison, a vector with RGB color characteristics was used (R mean, G mean, B mean), computing a $3$ dimension characteristics vector.

\subsection{Self-Organizing Maps (SOM)}
\todo{Add SOM explanation}

In Fig. \ref{fig:soms} a SOM of the fractal and non fractal feature vectors calculated for the scanner images are shown. On the one hand, the fractal features SOM seems to show easily separable classes, on the other hand, the RGB features appears to be more overlapped.

\begin{figure*}[]
\centering
\includegraphics[width=0.45\textwidth]{exps/som/som\mydot multifractal}
\includegraphics[width=0.45\textwidth]{exps/som/som\mydot rgb}
\caption{Self organizing maps for the Fractal and non Fractal features}
\label{fig:soms}
\end{figure*}
...
\begin{tabular}{|c|c|c|c|c|c|}
    ...
    \hline
\end{tabular}
\caption{Confusion Matrix for the fractal characteristics}
...
\begin{tabular}{|c|c|c|c|c|c|}
    ...
    \hline
\end{tabular}
\caption{Confusion Matrix for the RGB characteristics}
\label{table:ConfusionMatrixNonFractal}
\end{table}

\subsection{Cheating the classifier}
It is possible to cheat the classifiers with fake images. Fig. \ref{fig:fakeRGB} shows an image that is classified as bread by its RGB characteristics. Fig. \ref{fig:fakeMultifractal} shows an image generated by a particle system~\cite{Baravalle2011} which is classified as bread by its multifractal characteristics. It is easy to cheat the RGB classifier, using images with similar colors characteristics as breads, but it is not clear how to do that with fractal features. 
